...
public class DatasetsUtilTest {
    ...
    public void indexByNameTests() {
        ...
        Multimap<String, DatasetData> result = DatasetsUtil.indexByName(datasets);
        log.debug("{}", result);
        ...
        Collection<DatasetData> ds1Values = result.get("ds.1");
        assertNotNull(ds1Values, "Non-null values expected");
        assertReflectionEquals("Wrong values",
                               Lists.newArrayList(newDataset("ds.1"), newDataset("ds.1")),
                               ds1Values, LENIENT_ORDER);

        Collection<DatasetData> ds2Values = result.get("ds.2");
        assertNotNull(ds1Values, "Non-null values expected");
        assertReflectionEquals("Wrong values",
                               Lists.newArrayList(newDataset("ds.2")),
    ...
    public void indexByIdTests() {
        ...
        BiMap<Long, DatasetData> result = DatasetsUtil.indexById(datasets);
        log.debug("{}", result);
        ...
        DatasetData ds1 = result.get(1L);
        assertNotNull(ds1, "Non-null value expected");

        DatasetData ds2 = result.get(2L);
